...
public class ScheduleUser {
	...
	//------------------------------------------METODO PADEL GUARDAR REGISTRO y CONTADOR-------------------------------------
	public void bookingPadel(String id, String nombre) {
		try {
			Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost/desarrollosw", "root",
					"1234");
			//System.out.println("Connection Padel");
			java.sql.Statement stmt = conn.createStatement();
			String query1 = "INSERT INTO pruebauser " + "VALUES ('" + id + "', '" + nombre + "')";
			stmt.executeUpdate(query1);
			conn.close();
			
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "Error con la base de datos de inserccion de padel", "ERROR_MESSAGE", JOptionPane.ERROR_MESSAGE);
		}

	}
	
public int contPadel(String id) {
	
	int count = 0;
	try {
		 Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost/desarrollosw", "root", "1234");
	     //System.out.println("Connection contador padel");
	     java.sql.Statement stmt = conn.createStatement();
	     String query1 =  "SELECT count(id) FROM pruebauser where id like 'pa1_"+id+"'"; 
	     ResultSet rs = stmt.executeQuery(query1); 
		...
		JOptionPane.showMessageDialog(null, "Error con la base de datos de contaje", "ERROR_MESSAGE", JOptionPane.ERROR_MESSAGE);
	}
